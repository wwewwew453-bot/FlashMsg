...
# Використовуємо абсолютний шлях для SQLite на Render
...
def index():
    try:
        ...
        return f"Критична помилка шаблону: {e}. Перевір папку templates!"
...
def register():
    if request.method == 'POST':
        ...
        db.session.add(User(username=request.form.get('username'), password=pw))
        db.session.commit()
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all() # Створення бази при запуску
